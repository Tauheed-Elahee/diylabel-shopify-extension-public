/*
  # Complete DIY Label Database Setup
  
  This migration creates all required tables and sets up the database schema
  for the DIY Label Shopify app.
  
  1. Creates print_shops table (if not exists)
  2. Creates shopify_stores table
  3. Creates diy_label_orders table  
  4. Creates product_settings table
  5. Sets up Row Level Security
  6. Creates necessary indexes
  7. Inserts sample print shop data
*/

-- Create print_shops table (compatible with existing schema)
...
-- Add compatibility columns (latitude/longitude) if they don't exist
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Insert sample print shops data
INSERT INTO print_shops (name, lat, lng, address, specialty, rating, phone, email, website, capabilities, hours) VALUES
